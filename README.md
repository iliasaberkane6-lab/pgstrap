...
- `npm run db:generate` - Generate types and structure dumps against an in-memory PGlite instance, so a Postgres server is not required. Use `pgstrap generate --no-pglite` to generate from your configured Postgres database instead.
